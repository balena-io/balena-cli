...
export class SupervisorMock extends NockMock {
	...
	public expectGetPing(opts: ScopeOpts = {}) {
		...
		// .reply((uri, reqBody, cb) => {
		// 	console.error(`SupervisorMock.expectGetPing replying OK`);
		// 	cb(null, [200, 'OK']);
		// });
	}
	...
	public expectGetLogs(opts: ScopeOpts = {}) {
		...
		const chunkedStream = new Readable({
			read(_size) {
				// setImmediate(() => {
				setTimeout(() => {
					// this.push(chunkCount === chunks.length
					// 	? null
					// 	: chunks[chunkCount++]
					// );
					if (chunkCount === chunks.length) {
						// console.error(`SupervisorMock chunked stream push(null)`);
						this.push(null);
					} else {
						// console.error(`SupervisorMock chunked stream push(${chunks[chunkCount]})`);
						this.push(chunks[chunkCount++]);
					}
				}, 10);
			},
		});
		...
		this.optGet('/v2/local/logs', opts).reply((_uri, _reqBody, cb) => {
			// console.error(
			// 	`SupervisorMock.expectGetLogs replying with chunked stream`,
			// );
