...
export async function umount(device: string): Promise<void> {
	...
	if (process.platform === 'darwin') {
		umountCommand = '/usr/sbin/diskutil unmountDisk force';
	} else if (process.platform === 'linux') {
		const { whichBin } = await import('./which');
		umountCommand = await whichBin('umount');
	}

	if (umountCommand) {
		console.error(`umount: executing "${umountCommand}" for "${device}"`);
		await execFile(umountCommand, [device]);
	} else {
		console.error(`umount: umountCommand is falsy`);
	}
}
