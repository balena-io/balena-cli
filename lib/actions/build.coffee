...
removeSourceOption = (opts) ->
	_ = require('lodash')
	return _.reject(opts, signature: 'source')

module.exports =
	signature: 'build [source]'
	description: 'Build a single image or a multi-container project locally'
	permission: 'user'
	primary: true
	help: '''
	...
	'''
	options: removeSourceOption(dockerUtils.appendOptions compose.appendOptions [
		{
			signature: 'arch'
			parameter: 'arch'
	...
	action: (params, options, done) ->
		...
		Promise.try ->
			...
			if arch? and deviceType?
				...
				helpers.getAppInfo(application)
				.then (app) ->
					[ app.app_name, app.arch, app.device_type ]
