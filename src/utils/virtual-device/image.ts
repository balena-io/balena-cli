...
/**
 * Partition names that indicate an installation-media image.
 * Installation-media images contain the actual bootable OS in the /opt/ directory
...
/**
 * Result of installation-media image detection.
 */
export interface FlasherDetectionResult {
	/** Whether the image is an installation-media image */
	isFlasher: boolean;
...
/**
 * Detect if an image is an installation-media image.
 *
 * Installation-media images are self-installing images that contain a nested
 * disk-image in the `flash-rootA` partition's `/opt/` directory. The inner
 * image file has a `.balenaos-img` extension.
 *
 * @param imagePath - Path to the image file
...
export async function expandImage(
	...
	if (result.StatusCode !== 0) {
		throw new Error(
			`qemu-img resize failed with exit code ${result.StatusCode}`,
		);
	}
