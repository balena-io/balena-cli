...
export default class VirtualDeviceStartCmd extends Command {
	public static description = stripIndent`
		...
		COMPLETE WORKFLOW EXAMPLE:
		  ...
		  Step 1: Download a balenaOS image
		    $ balena os download generic-amd64 -o balena.img --version latest --type disk-image
	...
	private async restartInstance(
		...
		// Find the container
		const { instance: existingInstance } = await findContainer(identifier);
		...
		if (options.detached) {
			...
			await this.attachInteractive(instance.name);
		} else {
	...
	private async startNewVm(options: {
		...
		// Check for installation-media image - error if detected
		const flasherResult = await detectFlasherImage(imagePath);
		if (flasherResult.isFlasher) {
			throw new ExpectedError(
				`Installation-media images are not supported for virtual devices.\n` +
					`Virtual devices require a disk-image type.\n\n` +
					`Download a disk-image with:\n` +
					`  balena os download <DEVICE_TYPE> --type disk-image -o balena.img`,
			);
		}
		console.log('  Image type: OK (disk-image)');
	 ...
	 */
	private async attachInteractive(containerName: string) {
		console.log('\nKeyboard controls:');
		console.log(
