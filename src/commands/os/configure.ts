	...
	public static flags = (() => {
		const $flags = {
			...
			config: Flags.string({
				...
				// The only compatible flag with --config is system-connection, so we populate that at the end using code.
			}),
			...
			'device-type': Flags.string({
				...
				// We use validateOptions to provide a better error message than the one generated by oclif's dependsOn field
				// dependsOn: ['fleet'],
			}),
		...
		$flags.config.exclusive = Object.keys($flags).filter(
			(flagName) => flagName !== 'config' && flagName !== 'system-connection',
		);
		return $flags;
	})();
